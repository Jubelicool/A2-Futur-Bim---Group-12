...
def classifyFloors(floors,site_elev, model):
    ...
    for floor in floors:
        ...
        space_entities = outputVolumeArea(model)
        floor_entities+=6*"\t"+"<floor- class=\""+type+"\" name='{}'  level='{}' area = '{}' elev=\"{}\" >{}<span class=\"floor_stats\">{}</span> </floor->\n".format(floor.Name, level, space_entities, floor.Elevation, floor.Name, round(float(floor.Elevation),3))     
        level-=1
